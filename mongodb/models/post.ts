...
export interface IPostDocument extends IPost, IPostMethods {} // singular instance of post
...
// likePost method for Post object
...
// unlikePost method for Post object
...
PostSchema.methods.commentOnPost = async function (commentToAdd: ICommentBase) {
  try {
    ...
    console.log("Error: could not post comment", error);
  }
};
...
PostSchema.methods.getAllComments = async function () {
  try {
    ...
    console.log("Error: could not get comments", error);
  }
};
...
PostSchema.statics.getAllPosts = async function () {
  try {
    const posts = await this.find()
      .sort({ createdAt: -1 }) // get all posts, sorts by newest
      .populate({
        path: "comments",
    ...
    console.log("Error getAllPosts() definition in (posts.ts): ", error);
  }
};
